...
loginstatus=login.controller.login();
    ...
    if (loginstatus){
      out+=`<button onclick="dashboard.controller.view(1)" > ${svg.icon('house')} Dasboard</button>`;
    } else {
      out+=`<button onclick="login.controller.signup()" > ${svg.icon('person')} Daftar</button>
      <button onclick="login.controller.login()" >  ${svg.icon('lock')} Login</button>`;
    }
    ...
    <div class="row shadow">
      <div  class="col-1-3">
      <img src="/piawai/templates/donat/tema-race.png" class="img">
      </div>
...
d.gebi('content').innerHTML=home.view.home(1);
